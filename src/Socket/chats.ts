...
export const makeChatsSocket = (config: SocketConfig) => {
  ...
  const getAppStateSyncKey = async (keyId: string) => {
    const { [keyId]: key } = await sock.authState.keys.get(
      "app-state-sync-key",
      [keyId]
    );
    return key;
  ...
  const removeProfilePicture = async (jid: string) => {
    ...
    if (
      ...
      throw new Error(
        `The entered jid '${jid}' is not valid, enter your own jid or that of a group to continue`
      );
    }
    if (/^\d+@s\.whatsapp\.net$/.test(jid)) {
      if (jid !== jidNormalizedUser(sock.authState.creds.me.id)) {
        throw new Error(
          "You can only delete your own profile picture or the one of a group where you are an administrator"
        );
      }
    }
    await sock.query({
  ...
  const resyncAppState = sock.ev.createBufferedFunction(
      ...
      await sock.authState.keys.transaction(async () => {
        ...
        while (collectionsToHandle.size) {
          ...
          for (const name of collectionsToHandle) {
            const result = await sock.authState.keys.get(
            ...
            try {
              if (snapshot) {
                const { state: newState, mutationMap } =
                  await decodeSyncdSnapshot(
                    ...
                    config.appStateMacVerification.snapshot
                  );
              ...
              if (patches.length) {
                const { state: newState, mutationMap } = await decodePatches(
                  ...
                  initialVersionMap[name],
                  config.logger,
                  config.appStateMacVerification.patch
                );
                await sock.authState.keys.set({
                  "app-state-sync-version": { [name]: newState }
                });
  ...
  const profilePictureUrl = async (
    ...
    if (
      ...
      throw new Error(
        `The entered jid '${jid}' is not valid, please enter the jid of a person or group to continue`
      );
    }
    const result = await sock.query(
      {
  ...
  const presenceSubscribe = (jid: string, tcToken?: Buffer) =>
    sock.sendNode({
      tag: "presence",
      attrs: {
        to: jid,
  ...
  const appPatch = async (patchCreate: WAPatchCreate) => {
    ...
    await processingMutex.mutex(async () => {
      await sock.authState.keys.transaction(async () => {
        ...
        const { [name]: currentSyncVersion } = await sock.authState.keys.get(
          "app-state-sync-version",
          [name]
        );
  ...
  const upsertMessage = sock.ev.createBufferedFunction(
      ...
      async function doAppStateSync() {
        if (!sock.authState.creds.accountSyncCounter) {
          ...
          const accountSyncCounter =
            (sock.authState.creds.accountSyncCounter || 0) + 1;
      ...
      if (connection === "open") {
        if (config.fireInitQueries) {
          executeInitQueries().catch((error) =>
            sock.onUnexpectedError(error, "init queries")
          );
        }
        sendPresenceUpdate(
          config.markOnlineOnConnect ? "available" : "unavailable"
        ).catch((error) =>
          sock.onUnexpectedError(error, "presence update requests")
        );
      }
      if (
