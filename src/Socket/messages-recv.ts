...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
  ...
  const sendMessageAck = async (
    ...
    config.logger.debug(
      { recv: { tag, attrs }, sent: stanza.attrs },
      "sent ack"
    );
    ...
    switch (nodeType) {
      ...
      case "link_code_companion_reg":
        ...
        const companionSharedKey = Curve.sharedKey(
          sock.authState.creds.pairingEphemeralKeyPair.private,
        ...
        const identitySharedKey = Curve.sharedKey(
          sock.authState.creds.signedIdentityKey.private,
          primaryIdentityPublicKey
        );
  ...
  async function decipherLinkPublicKey(data: Uint8Array | Buffer) {
    ...
    const secretKey = await derivePairingCodeKey(
      sock.authState.creds.pairingCode,
      salt
    );
  ...
  const sendMessagesAgain = async (
    ...
    const msgs = await Promise.all(
      ids.map((id) => config.getMessage({ ...key, id }))
    );
  ...
  const handleReceipt = async (node: BinaryNode) => {
    ...
    const isNodeFromMe = areJidsSameUser(
      ...
      isLid ? sock.authState.creds.me?.lid : sock.authState.creds.me?.id
    );
    const remoteJid =
  ...
  const handleMessage = async (node: BinaryNode) => {
    if (
      config.shouldIgnoreJid(node.attrs.from) &&
  ...
  const fetchMessageHistory = async (
    ...
    return sock.sendPeerDataOperationMessage(
      pdoMessage as proto.Message.IPeerDataOperationRequestMessage
    );
  };
  ...
  const processNodeWithBuffer = async <T>(
    ...
    function execTask() {
      return exec(node, false).catch((err) =>
        sock.onUnexpectedError(err, identifier)
      );
    }
  };
  ...
  const makeOfflineNodeProcessor = () => {
    ...
    const enqueue = (type: MessageType, node: BinaryNode) => {
      ...
      promise().catch((error) =>
        sock.onUnexpectedError(error, "processing offline nodes")
      );
    };
    return { enqueue };
  };
  ...
  sock.ws.on("CB:ack,class:message", (node: BinaryNode) => {
    handleBadAck(node).catch((error) =>
      sock.onUnexpectedError(error, "handling bad ack")
    );
  });
