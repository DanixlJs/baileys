...
export const makeSocket = (config: SocketConfig) => {
  const url =
    typeof config.waWebSocketUrl === "string"
  ...
  if (url.protocol === "wss" && config.auth?.creds?.routingInfo) {
    url.searchParams.append(
      "ED",
      config.auth.creds.routingInfo.toString("base64url")
    );
  }
  ...
  const validateConnection = async () => {
    ...
    config.logger.info(
      { browser: config.browser, helloMsg },
  ...
  async function generatePairingKey() {
    ...
    const key = await derivePairingCodeKey(
      config.auth.creds.pairingCode!,
      salt
    );
    const ciphered = aesEncryptCTR(
      config.auth.creds.pairingEphemeralKeyPair.public,
      key,
      randomIv
    );
  ...
  ws.on("CB:ib,,edge_routing", (node: BinaryNode) => {
    ...
    if (routingInfo?.content) {
      config.auth.creds.routingInfo = Buffer.from(
