...
export const makeMessagesSocket = (config: SocketConfig) => {
  ...
  const assertSessions = async (jids: string[], force: boolean) => {
    ...
    if (force) {
      ...
      const addrs = jids.map((jid) =>
        sock.signalRepository.jidToSignalProtocolAddress(jid)
      );
    ...
    await sock.authState.keys.transaction(async () => {
      ...
      if (isGroup || isStatus) {
        const [groupData, senderKeyMap] = await Promise.all([
          (async () => {
            let groupData =
              useCachedGroupMetadata && config.cachedGroupMetadata
        ...
        const patched = await config.patchMessageBeforeSending(
          message,
          devices.map((d) =>
        ...
        const { ciphertext, senderKeyDistributionMessage } =
          await sock.signalRepository.encryptGroupMessage({
  ...
  const waitForMsgMediaUpdate = bindWaitForEvent(
    sock.ev,
    "messages.media-update"
  );

  const updateMediaMessage = async (message: proto.IWebMessageInfo) => {
  ...
  const sendMessage = async (
    ...
    if (
      ...
      if (config.emitOwnEvents) {
        process.nextTick(() => {
          sock.processingMutex.mutex(() =>
            sock.upsertMessage(fullMsg, "append")
          );
        });
      }
      return fullMsg;
