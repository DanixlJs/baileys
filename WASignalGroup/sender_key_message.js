...
export default class SenderKeyMessage extends CiphertextMessage {
  SIGNATURE_LENGTH = 64;
  constructor(keyId = null, iteration = null, ciphertext = null, signatureKey = null, serialized = null) {
    ...
    if (serialized) {
      ...
      const senderKeyMessage = protobufjs.decode(message).toJSON();
      ...
      const message = protobufjs.encode(protobufjs.create({
        id: keyId,
        iteration,
        ciphertext
      })).finish();
  ...
  getSignature(signatureKey, serialized) {
    const signature = Buffer.from(calculateSignature(signatureKey, serialized));
    return signature;
  }
  serialize() {
