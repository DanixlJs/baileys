...
export const makeGroupsSocket = (config) => {
    ...
    const groupQuery = async (jid, type, content) => sock.query({
        tag: "iq",
        attrs: {
            type,
            xmlns: "w:g2",
    ...
    const groupMetadata = async (jid) => {
        if (!jid || !/^\d+@g\.us$/.test(jid)) {
            throw new Error(`The entered jid '${jid}' is invalid, please enter a group jid to continue`);
        }
    ...
    const groupUpdateSubject = async (jid, subject) => {
        if (!jid || !/^\d+@g\.us$/.test(jid)) {
            throw new Error(`The entered jid '${jid}' is invalid, please enter a group jid to continue`);
        }
    ...
    const groupRequestParticipantsList = async (jid) => {
        if (!jid || !/^\d+@g\.us$/.test(jid)) {
            throw new Error(`The entered jid '${jid}' is invalid, please enter a group jid to continue`);
        }
        const result = await groupQuery(jid, "get", [
    ...
    const groupParticipantsUpdate = async (jid, participants, action) => {
        if (!jid || !/^\d+@g\.us$/.test(jid)) {
            throw new Error(`The entered jid '${jid}' is invalid, please enter a group jid to continue`);
        }
        const result = await groupQuery(jid, "set", [
    ...
    const groupUpdateDescription = async (jid, description) => {
        if (!jid || !/^\d+@g\.us$/.test(jid)) {
            throw new Error(`The entered jid '${jid}' is invalid, please enter a group jid to continue`);
        }
    ...
    const groupInviteCode = async (jid) => {
        if (!jid || !/^\d+@g\.us$/.test(jid)) {
            throw new Error(`The entered jid '${jid}' is invalid, please enter a group jid to continue`);
        }
    ...
    const groupInviteLink = async (jid) => {
        if (!jid || !/^\d+@g\.us$/.test(jid)) {
            throw new Error(`The entered jid '${jid}' is invalid, please enter a group jid to continue`);
        }
    ...
    const groupRevokeInvite = async (jid) => {
        if (!jid || !/^\d+@g\.us$/.test(jid)) {
            throw new Error(`The entered jid '${jid}' is invalid, please enter a group jid to continue`);
        }
    ...
    const groupAcceptInvite = async (code) => {
    ...
    const groupRevokeInviteV4 = async (groupJid, invitedJid) => {
        if (!groupJid || !/^\d+@g\.us$/.test(groupJid)) {
            throw new Error(`The entered jid '${groupJid}' is invalid, please enter a group jid to continue`);
        }
    ...
    const groupAcceptInviteV4 = sock.ev.createBufferedFunction(async (key, inviteMessage) => {
    ...
    const groupGetInviteInfo = async (code) => {
        const results = await groupQuery("@g.us", "get", [
            { tag: "invite", attrs: { code } }
        ]);
        return extractGroupMetadata(results);
    };
    const groupToggleEphemeral = async (jid, ephemeralExpiration) => {
        if (!jid || !/^\d+@g\.us$/.test(jid)) {
            throw new Error(`The entered jid '${jid}' is invalid, please enter a group jid to continue`);
        }
    ...
    const groupMemberAddMode = async (jid, mode) => {
        if (!jid || !/^\d+@g\.us$/.test(jid)) {
            throw new Error(`The entered jid '${jid}' is invalid, please enter a group jid to continue`);
        }
        await groupQuery(jid, "set", [
    ...
    const groupJoinApprovalMode = async (jid, mode) => {
        if (!jid || !/^\d+@g\.us$/.test(jid)) {
            throw new Error(`The entered jid '${jid}' is invalid, please enter a group jid to continue`);
        }
        await groupQuery(jid, "set", [
