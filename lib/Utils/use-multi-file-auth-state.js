...
export const useMultiFileAuthState = async (folder) => {
    ...
    const writeData = (data, file) => {
        ...
        return fileLock.acquire(filePath, () => writeFile(join(filePath), JSON.stringify(data, BufferJSON.replacer)));
    };
    const readData = async (file) => {
        try {
            const filePath = join(folder, fixFileName(file));
            const data = await fileLock.acquire(filePath, () => readFile(filePath, { encoding: "utf-8" }));
